...
bathy <- load_gebco_bathymetry(
  paste0(
    "~/Programming_Projects/Big_Data/gebco_2025_sub_ice_topo/",
...
iucn_shp <- paste0(
  "~/Programming_Projects/Big_Data/SHARKS_RAYS_CHIMAERAS/",
  "SHARKS_RAYS_CHIMAERAS.shp"
)
...
species_iucn_depth_iucn <- species_iucn %>% 
  select(
    scientific_name,
...
depth_refuge %>%
  ...
  select(
    species, fishery, area_overlap_prop, volume_overlap_prop, depth_refuge
  ) %>%
  head(10)
```
...
1. **Per-cell bathymetry clamping**: The original analysis applied a single
   ...
   depth_range). `ocean3d` clamps `depth_max` to the actual seafloor in
...
# (calc_volume_overlap returns volume rasters; extract area and depth
# separately)
...
# Volume comparison
plot(comparison$intersect_volume_m3 / 1e9,
     comparison$our_intersect_volume_m3 / 1e9,
...
fishery_richness_plots <- lapply(cr_species, function(sp) {
  ...
  ggplot() +
    ...
    geom_sf(data = land_mask, fill = "grey90", colour = "grey50",
            linewidth = 0.5) +
    ...
    annotate("rect",
             xmin = plot_bbox[["xmin"]] +
               0.02 * diff(plot_bbox[c("xmin", "xmax")]),
             xmax = plot_bbox[["xmin"]] +
               0.55 * diff(plot_bbox[c("xmin", "xmax")]),
             ymin = plot_bbox[["ymax"]] -
               0.05 * diff(plot_bbox[c("ymin", "ymax")]),
             ymax = plot_bbox[["ymax"]] -
               0.00 * diff(plot_bbox[c("ymin", "ymax")]),
             fill = "grey90", colour = NA) +
    annotate("text",
             x = plot_bbox[["xmin"]] +
               0.03 * diff(plot_bbox[c("xmin", "xmax")]),
             y = plot_bbox[["ymax"]] -
               0.02 * diff(plot_bbox[c("ymin", "ymax")]),
...
out_dir_fb <- here(
  "output", "bangladesh-fisheries-3d-overlap", "fishery_by_species"
)
...
grid_plots <- species_richness_plots[
  setdiff(names(species_richness_plots), "LGN")
]
...
out_dir_sb <- here(
  "output", "bangladesh-fisheries-3d-overlap", "species_by_fishery"
)
...
cr_overlap_rasters <- overlap_rasters[
  pairs$species %in% cr_species & pairs$fishery != "LGN"
]
```
...
names(overlap_map_plots) <- paste(cr_pairs$species, cr_pairs$fishery,
                                   sep = " x ")
```
...
overlap_depth_plots <- lapply(seq_len(nrow(cr_pairs)), function(i) {
  ...
  df <- as.data.frame(terra::values(stack)) %>%
    ...
    filter(sp_depth_min, fi_depth_min) %>%
  ...
  ggplot(df, aes(x = cell_index)) +
    ...
    annotate("text", x = max(df$cell_index) * 0.95,
             y = min(-df$seafloor_depth) * 0.95,
...
names(overlap_depth_plots) <- paste(cr_pairs$species, cr_pairs$fishery,
                                     sep = " x ")
```
...
out_dir_ov <- here(
  "output", "bangladesh-fisheries-3d-overlap", "species_fishery_overlap"
)
